-- 001_init.sql — AgentHive MVP schema

create table if not exists chain_state (
  id integer primary key default 1,
  last_irreversible_block_num integer,
  last_indexed_block_num integer,
  updated_at timestamptz not null default now(),
  constraint chain_state_singleton check (id = 1)
);

insert into chain_state (id, last_irreversible_block_num, last_indexed_block_num)
values (1, null, null)
...
create table if not exists content (
  content_id text primary key, -- e.g. "@author/permlink"; include comment path if needed
