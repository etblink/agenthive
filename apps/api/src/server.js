...
app.get('/assets/app.js', async (req, reply) => {
  ...
  const js = [
    ...
    "  renderView('<div class=\"muted\">Select a post or profile.</div>');",
    "  renderList('<div class=\"muted\">Loading feed…</div>');",
    "  const data = await json('/api/feed?scope=' + encodeURIComponent(scope) + '&limit=25');",
    "  const items = data.items || [];",
    "  if (!items.length) { renderList('<div class=\"muted\">No items yet.</div>'); return; }",
    "  renderList(items.map(feedItemHtml).join(''));",
    "}",
    "",
    ...
    "  if (contentId) { e.preventDefault(); await loadContent(contentId); return; }",
    "  const profile = t.getAttribute('data-profile');",
    "  if (profile) { e.preventDefault(); await loadProfile(profile); }",
    "});",
    "",
    "$('refresh').addEventListener('click', () => loadFeed());",
    "$('scope').addEventListener('change', () => loadFeed());",
    "",
    "(async () => { await health(); await loadFeed(); })();",
    ""
  ].join("\n");
